...
Verify: `ls .claude/rules/` → general.md, skill-routing.md, mcp-routing.md, token-efficiency.md present minimum. If `.mcp.json` exists at project root, `mcp-tool-routing.md` must ALSO be present (generated by Step 6 below). Absent `.mcp.json` → `mcp-tool-routing.md` absent is expected + not a failure.
...
### 6. Generate MCP Tool Routing Rule (Conditional on .mcp.json)

Main thread generates `.claude/rules/mcp-tool-routing.md` when `.mcp.json` exists at project root and declares serena or codebase-memory-mcp servers. No agent dispatch — single-file heredoc write.

**Detect MCP servers**:
```bash
if [[ ! -f .mcp.json ]]; then
  printf "SKIP: no .mcp.json — skipping mcp-tool-routing.md generation\n"
else
  HAS_SERENA=$(python3 -c "import json; d=json.load(open('.mcp.json')); print('yes' if 'serena' in d.get('mcpServers', {}) else '')" 2>/dev/null || echo "")
  HAS_CMM=$(python3 -c "import json; d=json.load(open('.mcp.json')); print('yes' if 'codebase-memory-mcp' in d.get('mcpServers', {}) else '')" 2>/dev/null || echo "")
  if [[ -z "$HAS_SERENA" && -z "$HAS_CMM" ]]; then
    printf "SKIP: .mcp.json present but no serena/codebase-memory-mcp — skipping mcp-tool-routing.md generation\n"
  else
    GENERATE_TOOL_ROUTING=yes
  fi
fi
```

**Write the rule file** (only if `GENERATE_TOOL_ROUTING=yes`). Use single-quoted heredoc to prevent variable expansion:

```bash
if [[ "${GENERATE_TOOL_ROUTING:-}" == "yes" ]]; then
  mkdir -p .claude/rules
  cat > .claude/rules/mcp-tool-routing.md.new <<'MCPRULE'
...
MCPRULE
  ...
  # Idempotency: overwrite only if content differs
  if [[ -f .claude/rules/mcp-tool-routing.md ]] && cmp -s .claude/rules/mcp-tool-routing.md .claude/rules/mcp-tool-routing.md.new; then
    printf "SKIP: .claude/rules/mcp-tool-routing.md already current\n"
    rm .claude/rules/mcp-tool-routing.md.new
  else
    mv .claude/rules/mcp-tool-routing.md.new .claude/rules/mcp-tool-routing.md
    printf "WROTE: .claude/rules/mcp-tool-routing.md\n"
  fi
fi
```

**Add matching `@import` to CLAUDE.md** (only when the rule file was created above — conditional on `.mcp.json`). Insert the import line immediately after the existing `@import .claude/rules/mcp-routing.md` line so the two MCP rules stay grouped:

```bash
if [[ "${GENERATE_TOOL_ROUTING:-}" == "yes" && -f CLAUDE.md ]]; then
  if ! grep -q "mcp-tool-routing.md" CLAUDE.md; then
    python3 - <<'PYEOF'
import re, pathlib
p = pathlib.Path("CLAUDE.md")
text = p.read_text(encoding="utf-8")
new_line = "@import .claude/rules/mcp-tool-routing.md"
# Preferred: insert immediately after the mcp-routing.md @import line
m = re.search(r"^(@import\s+\.claude/rules/mcp-routing\.md\s*)$", text, flags=re.MULTILINE)
if m:
    text = text[:m.end()] + "\n" + new_line + text[m.end():]
else:
    # Fallback: append after the last @import line in the file
    imports = list(re.finditer(r"^@import\s+[^\n]+$", text, flags=re.MULTILINE))
    if imports:
        last = imports[-1]
        text = text[:last.end()] + "\n" + new_line + text[last.end():]
    else:
        # Last-ditch: append to end of file
        text = text.rstrip() + "\n" + new_line + "\n"
p.write_text(text, encoding="utf-8")
print("ADDED: @import .claude/rules/mcp-tool-routing.md to CLAUDE.md")
PYEOF
  else
    printf "SKIP: CLAUDE.md already imports mcp-tool-routing.md\n"
  fi
fi
```

Idempotency: the heredoc write uses `cmp -s` to skip unchanged content; the CLAUDE.md patch is guarded by `grep -q` so repeated runs make no changes.
